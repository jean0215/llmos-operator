...
func (c *Client) ListObjects(ctx context.Context, collectionName, uid string, offset, limit int) (*vd.ObjectList, error) {
	...
	aggregateResult, err := c.weaviateClient.GraphQL().Aggregate().
...
// Query performs a vector search on the specified collection with the given query string, similarity threshold and limit
func (c *Client) Search(ctx context.Context, collectionName, queryString string, threshold float64, limit int) (*vd.SearchResults, error) {
	...
	if threshold > 0 {
		nearVectorBuilder.WithDistance(float32(threshold))
	}
