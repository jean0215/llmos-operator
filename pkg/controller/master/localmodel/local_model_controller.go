...
func (h *handler) OnChangeVersion(_ string, version *mlv1.LocalModelVersion) (*mlv1.LocalModelVersion, error) {
	...
	if err := h.setLatestAsDefaultVersion(version); err != nil {
		return version, fmt.Errorf("failed to set latest version as default version for local model version %s/%s: %w",
			version.Namespace, version.Name, err)
	}
...
func (h *handler) setLatestAsDefaultVersion(v *mlv1.LocalModelVersion) error {
